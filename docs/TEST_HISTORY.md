...
Source `7687c0a` reached Render in deployment `dep-danb4r68bjmc73adc5b0` at **16:05:14 UTC**. A copy of the earlier test-owned, already repaired installation successfully used the actual repair/update buttons, preserved the unfinished input and history, solved all three labs, and repeated those solves after a full browser shutdown/offline restart. [Report](evidence/2026-09-19/repeated-report/live-cached-upgrade.json). Its historical fault-injection label describes the original profile's provenance; **no new legacy worker was injected for this follow-up upgrade**.

The first fresh-profile entered-value run solved all three problems on both layouts but failed **both initial offline-preparation waits at 60 seconds**. These failures are [retained](evidence/2026-09-19/repeated-report/live-values-before-pool.log), not counted as offline successes. The desktop trace showed successful HTTP responses and an installation still downloading banks, not an unsupported solver request. [Actual edited-state-machine output](evidence/2026-09-19/repeated-report/live-edited-state-machine.png).

The downloader previously waited for each entire batch of three files, allowing one slow response to hold up subsequent downloads. It now uses three bounded download loops, immediately filling a slot when its file finishes. SHA-256 verification and the maximum of three concurrent downloads are unchanged. A new browser regression holds an early stylesheet download and requires a later demo bank to start before releasing it, while checking the concurrency bound. Final source share is **82.0219% C++ runtime / 77.0086% including HTML/CSS**. [Audit](evidence/2026-09-19/repeated-report/source-audit-pool.json). This removes head-of-line blocking; it does not guarantee a particular network download time.

The final local suite passes **100/100 browser tests in 2.7 minutes**. [Log](evidence/2026-09-19/repeated-report/browser-full-pool.log). Source `f45684c` reached Render in deployment `dep-danb837avr4c73a9hkug` at **16:12:21 UTC**; its served worker was checked to contain the bounded download loops. The visible offline asset-build suffix is `832ea0ef` (the worker-only scheduling change does not change the asset manifest).

The earlier UI-fix source `7687c0a` also passed [website CI](evidence/2026-09-19/repeated-report/website-ci-7687c0a.json) and all four [package CI jobs](evidence/2026-09-19/repeated-report/package-ci-7687c0a.json), including Android-emulator instrumentation. This is not physical-phone validation or a newly published installer release.

On the optimized live site, the phone-layout entered-value journey passed all three problems **online and offline** in 18.3 seconds total; [exact inputs and expected/actual results](evidence/2026-09-19/repeated-report/live-phone-entered-values.json), [offline convolution screenshot](evidence/2026-09-19/repeated-report/live-offline-convolution.png). The first desktop attempt still exceeded the strict 60-second first-download wait: [mixed-result log](evidence/2026-09-19/repeated-report/live-values-pool.log). Its trace showed the installation still downloading, with more banks reached before timeout; [trace summaries](evidence/2026-09-19/repeated-report/live-download-traces.json). This is not proof of a universal speed improvement under varying network/CDN conditions.

An **unchanged desktop repeat** passed all three entered problems online and offline in 8.4 seconds total, without relaxing the timeout or changing inputs. [Log](evidence/2026-09-19/repeated-report/live-desktop-repeat.log), [exact comparisons](evidence/2026-09-19/repeated-report/live-desktop-entered-values.json). Both passing reports contain zero page errors. The earlier timeouts remain failures, and the user's own reported unsupported request remains unverified pending their URL/device/error details.
...
The anonymous access failure is retained. A later read-only check at approximately **15:31 UTC** reported `isPrivate: false` / `visibility: PUBLIC` from both `gh repo view MuhammadTahaBinZaeem/FOP-Project` and the repository API. The agent did not change visibility and does not infer who or what did. Current release text reflects the new observed state. Render delivery remains independent of GitHub visibility; the earlier failed request and authenticated-only test limitations are not retroactively changed.
...
### Executed anonymous public-download checks

Render deployment `dep-daimmpijnfac73eckrj0` became live at 15:12:03 UTC, publishing source `16323a7` with unchanged release binaries built at `81feb66`.

- **5/5 actual browser download buttons** fetched complete APK/ZIP files without GitHub authentication; every SHA-256 matched the manifest, which also matched the staged manifest byte-for-byte. [Download results](evidence/2026-09-12/public-render-rc2/browser-downloads.json).
- **3/3 anonymous downloaded-desktop CI jobs** passed Windows x64, Ubuntu x64 and macOS arm64 execution. [Workflow 34701574653](https://github.com/MuhammadTahaBinZaeem/FOP-Project/actions/runs/34701574653), [retained workflow result](evidence/2026-09-12/anonymous-render-desktop-ci.json). The public Linux ZIP also passed unmodified on NixOS from a path with spaces. [Per-platform reports](evidence/2026-09-12/public-render-rc2/desktop/). The universal macOS binary contains Intel code, but this is not execution on an Intel Mac.
- The **publicly downloaded Release APK** installed and passed **4/4 instrumentation tests in 49.329 seconds**, with radios disabled, on the API35 x86_64 emulator. [Actual installation and test logs](evidence/2026-09-12/public-render-rc2/android/). This is not a physical ARM phone test or production signing.
- The **public website ZIP**, extracted and served by the publicly downloaded Linux server, passed **82/82 browser tests**. [Log](evidence/2026-09-12/public-render-rc2/website-tests.log).
- A fresh persistent browser installed the actual Render site, closed fully, restarted with networking disabled and solved natural-input, signals and 5,000 hard-demo questions at the same URL. The complete cache is **9,199,204 bytes / 184 files**, without installers. [Restart report](evidence/2026-09-12/public-render-rc2/offline-restart.json).
- An older test-owned installation upgraded through the **actual repair/update buttons**, acquired the new public download links, preserved its unsolved question during the update and subsequently solved after an offline reload. [Upgrade report](evidence/2026-09-12/public-render-rc2/cache-upgrade.json). The [initial test failure](evidence/2026-09-12/public-render-rc2/cache-upgrade-initial-test-failure.json) is retained: that harness incorrectly expected one-shot update-draft preservation to also autosave an unsolved question across a second ordinary refresh. The corrected test verifies the update contract, solves the restored draft, then explicitly enters a new offline question; unrestricted unsolved-draft autosave is not claimed.
- Build provenance, release notes, the 825,000-case comparison archive and the test-evidence ZIP all passed separate unauthenticated body-download/hash checks against staged bytes. [Supplement report](evidence/2026-09-12/public-render-rc2/supplement-downloads.json).

[Public validation summary](https://pocket-engineer.onrender.com/downloads/v0.5.0-rc2/PUBLIC_DOWNLOAD_VALIDATION.json) is a post-publication supplement. It does not replace or alter the tested installer/archive bytes. The earlier evidence ZIP is a build-validation snapshot; the post-publication results above are retained separately. Physical-device smoothness, development-signing limitations and measured emulator stalls remain open as documented below.
