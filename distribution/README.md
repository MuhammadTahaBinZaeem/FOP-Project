...
Only tested release artifacts belong in versioned `vX.Y.Z-rcN` directories here. The existing Render static build copies these verified files to generated `www/downloads/` for anonymous delivery, independently of source repository visibility. No repository visibility change or runtime GitHub token is required.
