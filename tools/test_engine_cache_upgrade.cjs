...
(async()=>{
 ...
 async function labs(page){
  ...
  await page.getByRole('button',{name:'RC low-pass',exact:true}).click();await page.locator('#circuit-analysis').selectOption('ac');
  await page.getByRole('button',{name:'Solve this circuit',exact:true}).click();await expect(page.locator('#answer')).toContainText('Node voltages');await expect(page.locator('#verification')).toContainText('KCL');
  await page.locator('.lab-tabs [data-lab=signals]').click();await page.locator('#signal-operation').selectOption('convolution');await page.locator('#signal-x').fill('1,2,3');await page.locator('#signal-h').fill('4,5');
  ...
  await page.locator('.lab-tabs [data-lab=fsm]').click();await page.getByRole('button',{name:'Generate diagram & equations',exact:true}).click();await expect(page.locator('#answer')).toContainText('Simulation outputs: 0 0 1 0 1');await expect(page.locator('#visual canvas')).toHaveCount(1);
 }
 try{
  ...
  if(phase==='seed'){
   ...
   await page.reload();await expect(page.locator('#solve')).toBeEnabled();await page.locator('.tool-launcher [data-lab=circuit]').click();await expect(page.locator('#lab-status')).toContainText('Unsupported engine request');
