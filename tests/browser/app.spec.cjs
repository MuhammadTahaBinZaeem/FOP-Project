...
async function select(page,domain,topic,input){await ui(page.locator('#domain')).selectOption(domain);await ui(page.locator('#topic')).selectOption(topic);await ui(page.locator('#input')).fill(input);await ui(page.locator('#solve')).click();await expect(page.locator('#result')).toBeVisible();}
...
test('history survives reload and can be cleared; no stored markup injection',async({page})=>{
  ...
  await page.reload();await expect(page.locator('#solve')).toBeEnabled();await ui(page.locator('.nav[data-view="history"]')).click();
  await expect(page.locator('.history-item')).toHaveCount(1);await ui(page.locator('.history-item')).click();
  await expect(page.locator('#input')).toHaveValue('7*8');
  await ui(page.locator('.nav[data-view="history"]')).click();await ui(page.locator('#clear-history')).click();await expect(page.locator('.history-item')).toHaveCount(0);
  await ui(page.locator('.nav[data-view="workbench"]')).click();await select(page,'algebra','numeric_evaluation','<img src=x onerror="window.injected=1">');
...
test('catalog search, keyboard submit, JSON export',async({page})=>{
  await ready(page);await ui(page.locator('.nav[data-view="subjects"]')).click();await ui(page.locator('#search')).fill('Karnaugh');
  await expect(page.locator('.topic-link')).toHaveCount(1);await ui(page.locator('.topic-link')).click();
  await ui(page.locator('#input')).press('Control+Enter');await expect(page.locator('#answer')).toHaveText('F = C');
  const download=page.waitForEvent('download');await ui(page.locator('#export')).click();expect((await download).suggestedFilename()).toBe('pocket-engineer-solution.json');
});
