...
for(const valid of [true,false])test(`repair ${valid?'reuses verified':'rejects corrupt'} assets from a previous cache without network`,async({page,context})=>{
  ...
  await context.setOffline(true);await ui(page.locator('.nav[data-view=downloads]')).click();await expect(page.locator('#retry-cache')).toBeEnabled();await ui(page.locator('#retry-cache')).click();await expect(page.locator('#retry-cache')).toBeEnabled({timeout:35000});
  ...
  if(valid){await page.reload();await expect(page.locator('#solve')).toBeEnabled();await page.locator('.nav[data-view=workbench]').click();await ui(page.locator('#input')).fill('23+19');await ui(page.locator('#solve')).click();await expect(page.locator('#answer')).toHaveText('42');}
});
test('readiness button repairs a missing cached solver before an offline revisit',async({page,context})=>{
  ...
  await ui(page.locator('.nav[data-view="downloads"]')).click();await ui(page.locator('#retry-cache')).click();
  ...
  await ui(page.locator('#input')).fill('twenty five plus seven');await ui(page.locator('#solve')).click();await expect(page.locator('#answer')).toHaveText('32');
});
...
test('explicit repair replaces corrupt cached content, not just missing files',async({page})=>{
  ...
  await ui(page.locator('.nav[data-view="downloads"]')).click();await ui(page.locator('#retry-cache')).click();await expect(page.locator('#retry-cache')).toBeEnabled({timeout:40000});await expect(page.locator('#cache-status')).toContainText('Ready offline');
