...
test('an update already installing at page startup exposes the real update button',async({page,context})=>{
  ...
  try{
    ...
    await page.reload();await expect(page.locator('#solve')).toBeEnabled();await page.locator('.nav[data-view=downloads]').click();
    ...
    await page.locator('.nav[data-view=workbench]').click();await page.locator('#input').fill('17+29');
    await expect(page.locator('#update-offline')).toBeVisible();
    await page.locator('.tool-launcher [data-lab=signals]').click();await page.locator('#signal-x').fill('9,8,7,6');
    await expect(page.locator('#update-offline')).toBeVisible();
    ...
    await context.setOffline(true);await page.locator('.nav[data-view=workbench]').click();await page.locator('#solve').click();await expect(page.locator('#answer')).toHaveText('46');
