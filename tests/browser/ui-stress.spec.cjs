...
async function solve(page,domain,topic,input){
  await ui(page.locator('#domain')).selectOption(domain);await ui(page.locator('#topic')).selectOption(topic);
  await ui(page.locator('#input')).fill(input);await ui(page.locator('#solve')).click();
...
test('every catalog topic through actual dropdowns, example and solve buttons',async({page})=>{
  ...
  for(const domain of domains){await ui(page.locator('#domain')).selectOption(domain);
    const topics=await page.locator('#topic option').evaluateAll(options=>options.map(o=>({value:o.value,title:o.textContent})));
    for(const topic of topics){await ui(page.locator('#topic')).selectOption(topic.value);await ui(page.locator('#example')).click();await ui(page.locator('#solve')).click();
      await expect(page.locator('#solve-form')).toHaveAttribute('aria-busy','false');
...
test('spacing and touch targets across phone, landscape, tablet and desktop views',async({page},info)=>{
  ...
  for(const [width,height] of [[320,568],[360,640],[390,844],[667,375],[768,1024],[1024,768],[1440,1000]]){
    ...
    for(const view of ['workbench','subjects','history','downloads']){
      await ui(page.locator(`.nav[data-view="${view}"]`)).click();
    ...
    if(info.project.name==='desktop'&&[320,390,768,1440].includes(width)){
      await ui(page.locator('.nav[data-view="workbench"]')).click();await page.screenshot({path:`test-results/layout-${width}.png`,fullPage:true});
    }
  }
  ...
  await ui(page.locator('.nav[data-view="workbench"]')).click();
...
test('trajectory labels retain CSS size after phone resize and hidden-view navigation',async({page},info)=>{
  ...
  await ui(page.locator('.nav[data-view="subjects"]')).click();await page.setViewportSize({width:360,height:640});
  await ui(page.locator('.nav[data-view="workbench"]')).click();
...
test('copy, save, print, identify, empty input, navigation and install feedback',async({page,context})=>{
  ...
  await ui(page.locator('#input')).fill('');await ui(page.locator('#solve')).click();await expect(page.locator('#notice')).toContainText('Enter a problem');
  await ui(page.locator('#input')).fill('2x+4=10');await ui(page.locator('#identify')).click();await expect(page.locator('#notice')).toContainText('Recognized:');await expect(page.locator('#topic')).toHaveValue('linear_equation');
  await solve(page,'algebra','numeric_evaluation','7*8');await ui(page.locator('#copy')).click();
  expect(await page.evaluate(()=>navigator.clipboard.readText())).toContain('56');
  const download=page.waitForEvent('download');await ui(page.locator('#export')).click();
  ...
  await page.evaluate(()=>{window.print=()=>window.printRequested=true;});await ui(page.locator('#print')).click();expect(await page.evaluate(()=>window.printRequested)).toBe(true);
  await solve(page,'algebra','numeric_evaluation','8*8');await expect(page.locator('#copy')).toHaveText('Copy solution');
  await ui(page.locator('.nav[data-view="subjects"]')).click();await ui(page.locator('#search')).fill('no-topic-matches-this');await expect(page.locator('#subject-list')).toContainText('No matching topics');
  await ui(page.locator('.nav[data-view="downloads"]')).click();await ui(page.locator('#retry-cache')).click();await expect(page.locator('#cache-status')).toContainText('Ready offline');
  ...
  await ui(page.locator('#install')).click();expect(await page.evaluate(()=>window.installRequested)).toBe(true);await expect(page.locator('#install')).toBeHidden();
...
test('UI endurance with independent expected answers, failures, offline reload and bounded history',async({page,context},info)=>{
  ...
  await ui(page.locator('.nav[data-view="history"]')).click();expect(await page.locator('.history-item').count()).toBeLessThanOrEqual(30);
