...
test('human question corrects a wrong type and preserves the original text',async({page})=>{
  ...
  await ui(page.locator('#domain')).selectOption('logic');await ui(page.locator('#topic')).selectOption('truth_table');
  ...
  await ui(page.locator('.nav[data-view="history"]')).click();await ui(page.locator('.history-item').first()).click();
...
test('manual mode stays explicit, then automatic interpretation can recover',async({page})=>{
  await page.goto('./');await expect(page.locator('#solve')).toBeEnabled();
  await ui(page.locator('#auto-type')).uncheck();await ui(page.locator('#input')).fill('Please calculate twenty five plus seven');await ui(page.locator('#solve')).click();
  ...
  await ui(page.locator('#auto-type')).check();await ui(page.locator('#solve')).click();await expect(page.locator('#answer')).toHaveText('32');
});
test('natural input works offline and ambiguity is visible without an invented answer',async({page,context})=>{
  ...
  await ui(page.locator('#input')).fill('Voltage divider Vin=12V, R1=1kOhm');await ui(page.locator('#solve')).click();
  await expect(page.locator('#interpretation')).toContainText('Please clarify');await expect(page.locator('#answer')).toContainText('r2');
  await ui(page.locator('#input')).fill('Voltage divider Vin=12V, R1=1kOhm, R2=2kOhm');await ui(page.locator('#solve')).click();
  await expect(page.locator('#answer')).toHaveText('Vout = 8 V; I = 0.004 A');
  await ui(page.locator('#input')).fill('Differentiate f(x) = 3x² - 2x + 7');await ui(page.locator('#solve')).click();
