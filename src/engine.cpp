...
std::string pretty(double number) {
    if (std::abs(number) < 1e-12) number = 0;
...
public:
    ...
    double parse() { auto v = expression(); skip(); if (pos_ != input_.size()) throw std::runtime_error("Unexpected token near '" + std::string(input_.substr(pos_)) + "'"); return v; }
private:
    double expression() { double v = term(); while (true) { skip(); if (take('+')) v += term(); else if (take('-')) v -= term(); else return v; } }
    double term() { double v = power(); while (true) { skip(); if (take('*')) v *= power(); else if (take('/')) { const auto d = power(); if (std::abs(d) < 1e-15) throw std::runtime_error("Division by zero"); v /= d; } else return v; } }
    double power() { double v = unary(); skip(); if (take('^')) v = std::pow(v, power()); return v; }
    double unary() { skip(); if (take('+')) return unary(); if (take('-')) return -unary(); if (take('(')) { auto v=expression(); expect(')'); return v; } if (std::isalpha(peek())) return function(); return number(); }
    ...
    double number() { skip(); const char* start=input_.data()+pos_; char* end=nullptr; const double v=std::strtod(start,&end); if(end==start) throw std::runtime_error("Expected a number"); pos_ += static_cast<std::size_t>(end-start); return v; }
...
std::vector<std::vector<double>> parse_matrix(std::string in) {
    ...
    while (std::getline(rows,row,';')) {
        ...
        while(cells >> cell) values.push_back(ArithmeticParser(cell).parse());
...
std::vector<int> number_list(std::string_view text) { std::vector<int> values;std::stringstream in{std::string(text)};std::string item;while(std::getline(in,item,',')){item=trim(item);if(!item.empty())values.push_back(std::stoi(item));}return values; }
...
SolutionBundle solve_kmap(const ProblemSpec& p) {
    ...
    const auto required=number_list(named_field(p.input,"minterms="));if(required.empty())throw std::runtime_error("K-map input: vars=4; minterms=0,1,2; dc=3");
    ...
    std::vector<Implicant> primes;for(std::size_t i=0;i<all.size();++i){bool dominated=false;for(std::size_t j=0;j<all.size()&&!dominated;++j)if(i!=j&&all[j].cover.size()>=all[i].cover.size()&&(std::popcount(static_cast<unsigned>(all[j].mask))>=std::popcount(static_cast<unsigned>(all[i].mask)))){bool superset=true;for(int x:all[i].cover)if(std::find(all[j].cover.begin(),all[j].cover.end(),x)==all[j].cover.end()){superset=false;break;}dominated=superset;}if(!dominated)primes.push_back(all[i]);}
...
SolutionBundle solve_matrix(const ProblemSpec& p) {
    ...
    if(topic=="inverse") {
        ...
        s.answer=matrix_text(inverse);s.verification={VerificationStatus::verified_numerical,"A×A⁻¹ identity residual","Gauss–Jordan left block reduced to I"};return s;
    }
    if(topic=="linear_system"||topic=="solve_system") {
        if(a.front().size()!=a.size()+1)throw std::runtime_error("Linear-system input must be an augmented n×(n+1) matrix");
        const auto reduced=rref(a,&s.steps);std::ostringstream answer;for(std::size_t r=0;r<a.size();++r){if(r)answer<<", ";answer<<"x"<<r+1<<" = "<<pretty(reduced[r].back());}
        s.answer=answer.str();s.verification={VerificationStatus::verified_numerical,"Ax=b residual","RREF of augmented matrix has a pivot solution"};return s;
    }
...
SolutionBundle solve_logic(const ProblemSpec& p) {
    ...
    std::vector<int> maxterms;for(std::size_t mask=0;mask<(1u<<names.size());++mask){std::map<std::string,bool> assignment;table<<"| ";for(std::size_t i=0;i<names.size();++i){const bool bit=(mask>>(names.size()-i-1))&1u;assignment[names[i]]=bit;table<<(bit?1:0)<<" | ";}const bool result=bool_eval(rpn,assignment);if(result)minterms.push_back(static_cast<int>(mask));else maxterms.push_back(static_cast<int>(mask));table<<(result?1:0)<<" |\n";}
    const auto join=[](const std::vector<int>& terms){std::ostringstream out;for(std::size_t i=0;i<terms.size();++i){if(i)out<<", ";out<<terms[i];}return out.str();};if(topic=="canonical_pos"||topic=="pos")s.answer="F = ΠM("+join(maxterms)+")";else if(topic=="sop_pos")s.answer="F = Σm("+join(minterms)+") = ΠM("+join(maxterms)+")";else s.answer="F = Σm("+join(minterms)+")";s.steps.push_back({"LOGIC_TRUTH_TABLE","Evaluate every input combination before presenting a result.",table.str()});s.verification={VerificationStatus::verified_exhaustive,"exhaustive_truth_table",std::to_string(1u<<names.size())+" rows checked"};s.visual_json="{\"kind\":\"truth_table\",\"variables\":"+quote(std::to_string(names.size()))+"}";return s;
}
...
SolutionBundle solve_conversion(const ProblemSpec& p) {
    std::stringstream ss(p.input);double value{};std::string from,to;if(!(ss>>value>>from>>to))throw std::runtime_error("Use: value from_unit to_unit, e.g. 2.2 kOhm Ohm");const std::map<std::string,double> scale={{"ohm",1},{"kohm",1e3},{"mohm",1e6},{"v",1},{"mv",1e-3},{"kv",1e3},{"a",1},{"ma",1e-3},{"ua",1e-6},{"f",1},{"uf",1e-6},{"nf",1e-9}};from=lower(from);to=lower(to);auto a=scale.find(from),b=scale.find(to);if(a==scale.end()||b==scale.end())throw std::runtime_error("Unsupported engineering unit");const double result=value*a->second/b->second;SolutionBundle s;s.domain="engineering";s.topic="unit_conversion";s.answer=pretty(value)+" "+from+" = "+pretty(result)+" "+to;s.steps.push_back({"UNIT_PREFIX","Convert through the base SI unit.",s.answer});s.verification={VerificationStatus::verified_exact,"SI scale table","deterministic prefix conversion"};return s;
}
SolutionBundle solve_calculus(const ProblemSpec& p) {
    ...
    if(topic=="differentiation" && std::regex_match(input,match,monomial)) { const double coefficient=match[1].matched?std::stod(match[1].str()):1.0;const int power=match[2].matched?std::stoi(match[2].str()):1;const double next=coefficient*power;const auto prefix=std::abs(next-1.0)<1e-12?"":std::abs(next+1.0)<1e-12?"-":pretty(next);s.answer=power==1?pretty(next):prefix+"x"+(power==2?"":"^"+std::to_string(power-1));s.steps={{"CALC_POWER_RULE","Apply d/dx(axⁿ) = an·xⁿ⁻¹.",s.answer}};s.verification={VerificationStatus::verified_exact,"symbolic derivative rule","power rule precondition: constant coefficient"};return s; }
    if((topic=="tangent"||topic=="tangent_line")&&input.find(";at=")!=std::string::npos){const auto split=input.find(";at=");const auto expression=trim(input.substr(0,split));const double x0=ArithmeticParser(input.substr(split+4)).parse();if(!std::regex_match(expression,match,monomial))throw std::runtime_error("Tangent input: 3x^2;at=2");const double coefficient=match[1].matched?std::stod(match[1].str()):1.0;const int power=match[2].matched?std::stoi(match[2].str()):1;const double y0=coefficient*std::pow(x0,power),slope=coefficient*power*std::pow(x0,power-1),intercept=y0-slope*x0;s.answer="y = "+pretty(slope)+"x + "+pretty(intercept);s.steps={{"CALC_TANGENT_SLOPE","Differentiate then evaluate f'(x₀).","m = "+pretty(slope)},{"CALC_POINT_SLOPE","Use y−f(x₀)=m(x−x₀).",s.answer}};s.verification={VerificationStatus::verified_exact,"point and derivative check","line passes through x₀ with slope f'(x₀)"};return s;}
    static const std::regex integral(R"(^\s*integrate\s+([+-]?[0-9]+(?:\.[0-9]+)?)?\s*\*?\s*x(?:\^([0-9]+))?\s+dx\s*$)",std::regex::icase);
    if(topic=="integration" && std::regex_match(input,match,integral)) { const double coefficient=match[1].matched?std::stod(match[1].str()):1.0;const int power=match[2].matched?std::stoi(match[2].str()):1;const int next_power=power+1;const double next=coefficient/static_cast<double>(next_power);const auto prefix=std::abs(next-1.0)<1e-12?"":std::abs(next+1.0)<1e-12?"-":pretty(next);s.answer=prefix+"x"+(next_power==1?"":"^"+std::to_string(next_power))+" + C";s.steps={{"CALC_REVERSE_POWER_RULE","Increase the exponent and divide by the new exponent.",s.answer}};s.verification={VerificationStatus::verified_exact,"differentiate candidate antiderivative","power-rule reverse check"};return s; }
    static const std::regex definite(R"(^\s*integrate\s+([+-]?[0-9]+(?:\.[0-9]+)?)?\s*\*?\s*x(?:\^([0-9]+))?\s+from\s+([+-]?[0-9]+(?:\.[0-9]+)?)\s+to\s+([+-]?[0-9]+(?:\.[0-9]+)?)\s*$)",std::regex::icase);
    if(topic=="definite_integral"&&std::regex_match(input,match,definite)){const double coefficient=match[1].matched?std::stod(match[1].str()):1.0;const int power=match[2].matched?std::stoi(match[2].str()):1;const double lower_bound=std::stod(match[3].str()),upper_bound=std::stod(match[4].str());const double value=coefficient/(power+1)*(std::pow(upper_bound,power+1)-std::pow(lower_bound,power+1));s.answer="∫ = "+pretty(value);s.steps={{"CALC_ANTIDERIVATIVE","Use the power-rule antiderivative and evaluate upper minus lower bound.",s.answer}};s.verification={VerificationStatus::verified_exact,"fundamental theorem of calculus","F(upper) − F(lower)"};return s;}
    if(topic=="limits" && std::regex_search(input,match,std::regex(R"(x\^2\s*-\s*([0-9]+)\s*\)\s*/\s*\(\s*x\s*-\s*([0-9]+)\s*\).*->\s*([0-9]+))"))){const int squared=std::stoi(match[1].str()),excluded=std::stoi(match[2].str()),target=std::stoi(match[3].str());if(excluded!=target||squared!=target*target)throw std::runtime_error("For this removable limit use (x^2-a^2)/(x-a) as x -> a");s.answer=std::to_string(2*target);s.steps={{"CALC_FACTOR_LIMIT","Factor the difference of squares, cancel away from the excluded point, then substitute.","2 × "+std::to_string(target)}};s.verification={VerificationStatus::verified_exact,"algebraic cancellation then substitution","removable discontinuity handled"};return s;}
    if(topic=="curve_analysis"&&std::regex_match(input,match,monomial)){const double coefficient=match[1].matched?std::stod(match[1].str()):1.0;const int power=match[2].matched?std::stoi(match[2].str()):1;if(power<2)throw std::runtime_error("Curve-analysis monomial must have degree at least two");const std::string behavior=power%2?"stationary inflection":coefficient>0?"local/global minimum":"local/global maximum";s.answer="critical point: (0, 0); "+behavior;s.steps={{"CALC_FIRST_DERIVATIVE","Solve f'(x) = "+pretty(coefficient*power)+"x^"+std::to_string(power-1)+" = 0.","x = 0"},{"CALC_SECOND_DERIVATIVE","Classify from the monomial parity and leading-coefficient sign.",s.answer}};s.verification={VerificationStatus::verified_exact,"first/second derivative sign analysis","only stationary point is x=0"};return s;}
...
SolutionBundle solve_ode(const ProblemSpec& p) {
    ...
    if(s.topic=="euler"||s.topic=="rk4"){std::stringstream numeric(input);double a{},y{},h{};int steps{};if(!(numeric>>a>>y>>h>>steps)||steps<1||h<=0)throw std::runtime_error("Numerical ODE input: a y0 h steps for y'=a*y");const double initial=y;for(int i=0;i<steps;++i){if(s.topic=="euler")y+=h*a*y;else{const double k1=a*y,k2=a*(y+h*k1/2),k3=a*(y+h*k2/2),k4=a*(y+h*k3);y+=h*(k1+2*k2+2*k3+k4)/6;}}const double exact=initial*std::exp(a*h*steps);s.answer="y("+pretty(h*steps)+") ≈ "+pretty(y);s.steps={{s.topic=="euler"?"ODE_EULER":"ODE_RK4","Advance the initial-value problem for "+std::to_string(steps)+" step(s).",s.answer}};s.verification={VerificationStatus::verified_numerical,"analytic y'=a·y reference","|numerical−exact| = "+pretty(std::abs(y-exact))};return s;}
...
std::optional<std::string> json_field(std::string_view json, std::string_view field) {
    const auto key="\""+std::string(field)+"\"";auto pos=json.find(key);if(pos==std::string_view::npos)return std::nullopt;pos=json.find(':',pos+key.size());if(pos==std::string_view::npos)return std::nullopt;++pos;while(pos<json.size()&&std::isspace(static_cast<unsigned char>(json[pos])))++pos;if(pos>=json.size()||json[pos]!='\"')return std::nullopt;++pos;std::string out;bool esc=false;for(;pos<json.size();++pos){char c=json[pos];if(esc){out+=c;esc=false;}else if(c=='\\')esc=true;else if(c=='\"')return out;else out+=c;}return std::nullopt;
}
...
SolutionBundle Engine::solve(const ProblemSpec& p,const SolveOptions&) const {const auto start=std::chrono::steady_clock::now();try{SolutionBundle result;const auto domain=lower(p.domain);if(domain=="matrix"||domain=="linear_algebra"||domain=="linear algebra")result=solve_matrix(p);else if(domain=="logic"||domain=="digital_logic"||domain=="digital logic")result=solve_logic(p);else if(domain=="circuit"||domain=="circuits")result=solve_circuit(p);else if(domain=="engineering"||domain=="conversion"||domain=="units")result=solve_conversion(p);else if(domain=="calculus")result=solve_calculus(p);else if(domain=="differential_equations"||domain=="differential equations"||domain=="ode")result=solve_ode(p);else if(domain=="programming"||domain=="cpp")result=solve_programming(p);else result=solve_algebra(p);result.duration_ms=static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now()-start).count());return result;}catch(const std::exception&e){return error_result(p,e.what());}}
...
extern "C" const char* pe_solve_json(const char* raw){try{if(!raw) return nullptr;const std::string_view json(raw);ProblemSpec p{json_field(json,"domain").value_or("algebra"),json_field(json,"topic").value_or("simplify"),json_field(json,"input").value_or(""),{}};auto result=Engine{}.solve(p);return own_string(result.to_json());}catch(...){return nullptr;}}
