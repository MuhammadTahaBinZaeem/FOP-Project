...
SolutionBundle solve_kmap(const ProblemSpec& p) {
    const auto vars_text=named_field(p.input,"vars=");const int variables=vars_text.empty()?4:std::stoi(vars_text);if(variables<2||variables>6)throw std::runtime_error("Offline K-map minimization supports 2–6 variables");
    if(variables>4)return workbench::extended_kmap(p);
...
std::string Engine::capabilities_json() const {return R"({"schema_version":"2.0","offline":true,"domains":[{"id":"algebra","topics":["numeric_evaluation","simplification","factorisation","linear_equations","quadratic_equations","trigonometry","logarithms"]},{"id":"calculus","topics":["differentiation","tangent_line","integration","definite_integral","limits","curve_analysis"]},{"id":"linear_algebra","topics":["rref","determinant","inverse","linear_system","multiply","transpose","rank","eigenvalues","vectors"]},{"id":"differential_equations","topics":["separable","first_order_linear","exact","bernoulli","homogeneous","second_order_constant_coefficient","initial_value","euler","rk4"]},{"id":"logic","topics":["number_systems","signed_arithmetic","truth_table","canonical_pos","kmap_minimization","combinational_logic","sequential_logic"]},{"id":"circuit","topics":["voltage_divider","dc_nodal_analysis","mesh_analysis","source_transformation","superposition","thevenin","norton","maximum_power","rc_transient","rl_transient"]},{"id":"engineering","topics":["unit_conversion"]},{"id":"programming","topics":["cpp_trace","branches","loops","arrays","functions","recursion"]}],"limits":{"truth_table_variables":6,"kmap_variables":6,"matrix_dimensions":"bounded by validated input budget","programming_loop_iterations":100000,"recursion_factorial_n":12}})";}
...
extern "C" const char* pe_workbench_json(const char* raw){try{if(!raw)throw std::runtime_error("Missing workbench request");return own_string(workbench::dispatch(parse_request(raw)));}catch(const std::exception& e){return own_string(error_result({},e.what()).to_json());}catch(...){return nullptr;}}
